...
function Channel() {
	...
	useEffect(() => {
		...
		const vid = async () => {
			...
			try {
				const res = await axios.get(`http://localhost:5000/api/v1/channel/${channelId[1]}`, { withCredentials: true })
	...
	const fetchVideos = async () => {
		...
		try {
			const res = await axios.post(`http://localhost:5000/api/v1/channel${videoTag}?pageToken=${nextPageToken || ""}`, { id: id.id }, { withCredentials: true })
			console.log(res.data)
			if (active === "Videos") {
